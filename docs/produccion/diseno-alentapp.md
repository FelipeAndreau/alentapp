...
## 8. Extensión de Observabilidad — Entidades Sport y Enrollment (MaxiC55)

### 8.1 Métricas RED — SportController

Los 4 endpoints de la entidad Sport se instrumentan siguiendo el mismo patrón de `PaymentController`:

| Endpoint                    | Rate                                    | Errors                                   | Duration                      |
| --------------------------- | --------------------------------------- | ---------------------------------------- | ----------------------------- |
| `GET /api/v1/sports`        | `http.requests.total` +1 con status 200 | 500 → `http.requests.errors`             | `http.request.duration` en ms |
| `POST /api/v1/sports`       | status 201                              | 400 (validación) / 409 (duplicado) / 500 | ídem                          |
| `PATCH /api/v1/sports/:id`  | status 200                              | 404 (no encontrado) / 400 / 500          | ídem                          |
| `DELETE /api/v1/sports/:id` | status 200                              | 404 (no encontrado) / 500                | ídem                          |

**Archivo:** `packages/api/src/delivery/sports/SportController.ts`

### 8.2 Métricas RED — EnrollmentController

Los 4 endpoints de la entidad Enrollment:

| Endpoint                         | Rate       | Errors                                                  | Duration                |
| -------------------------------- | ---------- | ------------------------------------------------------- | ----------------------- |
| `GET /api/v1/enrollments`        | status 200 | 500                                                     | `http.request.duration` |
| `POST /api/v1/enrollments`       | status 201 | 400 / 409 (duplicado, cupo lleno, socio inactivo) / 500 | ídem                    |
| `PATCH /api/v1/enrollments/:id`  | status 200 | 404 / 409 (ya eliminado) / 500                          | ídem                    |
| `DELETE /api/v1/enrollments/:id` | status 200 | 404 / 409 (ya eliminado) / 500                          | ídem                    |

**Archivo:** `packages/api/src/delivery/enrollments/EnrollmentController.ts`

### 8.3 Dashboards Grafana

**`red-sports.json`** — 6 paneles filtrando por `route=~"/api/v1/sports.*"`:

1. Requests por segundo (time series)
2. Tasa de error % (time series con thresholds verde/amarillo/rojo)
3. Latencia p95 / p99 (time series)
4. Requests por código de estado (stacked area)
5. Memoria del proceso en MB (time series)
6. Endpoints más lentos — top 5 (bargauge horizontal)

**`red-enrollments.json`** — mismos 6 paneles filtrando por `route=~"/api/v1/enrollments.*"`

### 8.4 Alertas Prometheus

Agregadas al `observability/prometheus/rules.yml` (grupo `alentapp-sports` y `alentapp-enrollments`):

| Alert                     | Condición                                       | Severidad |
| ------------------------- | ----------------------------------------------- | --------- |
| `SportHighErrorRate`      | error rate > 5% por 2 minutos en `/sports`      | warning   |
| `SportHighLatency`        | p95 > 1000ms por 2 minutos en `/sports`         | warning   |
| `EnrollmentHighErrorRate` | error rate > 5% por 2 minutos en `/enrollments` | warning   |
| `EnrollmentHighLatency`   | p95 > 1000ms por 2 minutos en `/enrollments`    | warning   |

### 8.5 Seguridad — Helmet y Rate Limiting

Además de los cambios de seguridad de Pedro (CORS, healthchecks, puerto 9464 interno), se agregan dos plugins de Fastify que faltaban:

**`@fastify/helmet`** — agrega headers HTTP de seguridad automáticamente:

- `X-Frame-Options: SAMEORIGIN` — previene clickjacking
- `X-Content-Type-Options: nosniff` — previene MIME sniffing
- `X-XSS-Protection` — activa protección XSS del browser
- `Strict-Transport-Security` — fuerza HTTPS
- Entre otros ~8 headers más

**`@fastify/rate-limit`** — limita 100 requests por minuto por IP:

- Si se supera el límite, Fastify responde automáticamente con `429 Too Many Requests`
- Protege contra fuerza bruta y abuso de la API

**Archivos modificados:** `packages/api/src/app.ts`, `packages/api/package.json`
